...
from ..load_tools import load_prompt, interpolate, unwarp
...
class SimpleInputArgs(BaseModel):
    ...
    image: Optional[list | str] = Field(default=None, description="图片")
...
class LLMPrompt(Node):
    ...
    - 输入可以是字符串（简单文本），也可以是字典（包含任务和参数）。对于字典类型的输入，类会自动验证并处理相应的任务和检索（retriever）。
      消息管理：
    - PromptRole 类用于管理不同角色的消息（如用户消息、系统消息、上下文消息等），并支持根据输入生成相应的消息对象。
      扩展性和灵活性：
    ...
    def from_template(cls, template: str, **kwargs):
        ...
        if instance._template != "invalid label":
            ...
            ########################### 从字符串新建提示词模板核心逻辑 ##############################
             ...
             2、根据参数列表和模板内容生成 OmegaConf 格式的模板对象
            ...
            def to_omega(match_obj):
                """ 将模板中的占位符转换为 Omegaconf 引用 """
                content = match_obj.group(0)[1:-1]  # 获取 {} 内的内容，去掉 {} 
                if content != "image":  # 对于非图片参数，添加 Omegaconf 引用
            ...
            omega = re.sub(r'\{.*?\}', to_omega, template)  # 将占位符替换为 Omegaconf 引用
            if "image" not in args:
                print("[LLMPrompt.from_template] 纯文本信息")
                omega_msg = [PromptRole.mask(), PromptRole.user(omega)]
            ...
            instance._template = OmegaConf.create({# 创建 OmegaConf 格式的模板对象
                "message": omega_msg,
                "meta": {
                    "retriver_call": None,
                    "model_args": None,
    ...
    @cli_visualize
    def operate(
        self, input: Union[str, dict], config: RTConfig = None, *args, **kwargs
    ) -> NodeCompletion:
        ...
        def _simple_invoke(input, config = None, *args, **kwargs) -> list:
            ...
            if isinstance(input, str):
                self.message.append(PromptRole.user(input))
                return self.message
                ...
                self.message.append(PromptRole.context(retrieved))
                self.message.append(PromptRole.user(input["task"]))
                ...
                for arg, value in input.items():            # 遍历传入参数
                    ...
                    if arg == "image" and value:            # 处理图片参数
                        if not isinstance(value, list):     # 如果不是列表，则转换为列表
                            value = [value]
                        self._template.message = [PromptRole.user_vision(# 使用包含视觉信息的用户消息
        ...
        return NodeCompletion(BuiltInType.LLMIP, output_messages, config=config)
