...
class NodeChain(Node):
    ...
    def handle_callback(self, node: dict):
        ...
        if node_output.type != NodeType.WRAPP: # 若不是封装节点（见 encapsulate 函数），则需要处理回调函数
            # 获取回调函数的参数
    ...
    if isinstance(obj, Node):   # 其 operate 函数的输出类型为 WRAPPED，代表是封装节点
        ...
        class SimpleNode(Node):
            ...
            def operate(
                ...
                return NodeCompletion(NodeType.WRAPP, obj(input, config, *args, **kwargs))
        result = SimpleNode()
        ...
        class SimpleNode(Node):
            ...
            def operate(
                ...
                return NodeCompletion(NodeType.WRAPP, obj)
        result = SimpleNode()
    else:
        ...
        class SimpleNode(Node):
            ...
            def operate(
                ...
                if hasattr(obj, "operate"):  # 如果对象有 operate 方法，则调用该方法
                    return NodeCompletion(NodeType.WRAPP, obj.operate(input, config, *args, **kwargs))
                else:
                    return NodeCompletion(NodeType.WRAPP, obj)  #  否则返回对象本身
        result = SimpleNode()
    return result
