...
from ...catena_core.callback.node_callback import (
    NodeCallback, 
    node_callback
)
...
class OpenAIC(OpenAI, Node):
    ...
    def query(self, message: List[Dict], **kwargs) -> Dict:
        ...
        return MessageRole.assistant(result) 
    ...
    def operate(
        ...
        reply = completion.choices[0].message
        llmreply = MessageRole.assistant(reply.content)

        # 返回处理结果
        debug("[models] related_chain_ids:", self.related_chain_ids)
        if NodeType.MEM in self.related_chain_ids:
...
class Qwen(OpenAI, Node):
    ...
    def regenerate(self, *args, **kwargs) -> NodeBus:
        self._signature = self.get_name() + "(CallBack: regenerate)"
        regen:NodeBus = self.operate(self._cache, *args, **kwargs)
